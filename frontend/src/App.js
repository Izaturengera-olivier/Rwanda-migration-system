...
function NavBar({ adminUser, onLogout }) {
    ...
    return ( <
        ...
        sx = {
            { flexGrow: 1, cursor: 'pointer', fontSize: { xs: '0.85rem', md: '1.1rem' } }
        }
        onClick = {
            () => navigate('/')
        } >
            ...
            isMobile ? ( <
                ...
                Box sx = {
                    ...
                    adminUser && ( <
                        >
                        <
                        Chip label = { adminUser.username }
                        size = "small"
                        sx = {
                            { bgcolor: 'rgba(255,255,255,0.2)', color: 'white', ml: 1 }
                        }
                        /> <
...
function App() {
    ...
    useEffect(() => {
        axios.get(`${API_BASE}/auth/login/`, { withCredentials: true })
            .then(res => { if (res.data && res.data.is_admin) setAdminUser(res.data); })
            .catch(() => {});
    }, []);
    ...
    return ( <
        ...
        /> <
        Route path = "/map"
        element = { < MigrationRiskMap / > }
        /> <
        Route path = "/district/:id"
        element = { < DistrictProfile / > }
        /> <
        Route path = "/infrastructure"
        element = { < InfrastructureGaps / > }
        /> <
        Route path = "/compare"
        element = { < CompareAreas / > }
        /> <
        Route path = "/trends"
        element = { < Trends / > }
        /> <
        ...
        /> <
        Route path = "/admin"
        element = { < ProtectedAdmin adminUser = { adminUser }
            onLoginSuccess = { setAdminUser }
            />} / >
            <
            Route path = "*"
            element = { < Navigate to = "/"
            ...
            sx = {
                { py: 2, textAlign: 'center', bgcolor: 'grey.100' }
            } >
            <
