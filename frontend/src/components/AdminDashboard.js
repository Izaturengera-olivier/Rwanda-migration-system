...
function AdminDashboard({ adminUser }) {
    ...
    const fetchDatasets = () =>
        axios.get(`${API_BASE}/datasets/`, { headers: getAuthHeaders(), withCredentials: true })
            .then(r => setDatasets(r.data.results || r.data))
            .catch(() => {});

    const fetchModels = () =>
        axios.get(`${API_BASE}/models/`, { headers: getAuthHeaders(), withCredentials: true })
            .then(r => setModels(r.data.results || r.data))
            .catch(() => {});

    const fetchAuditLogs = () =>
        axios.get(`${API_BASE}/audit-logs/`, { headers: getAuthHeaders(), withCredentials: true })
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.error || err.response?.data?.detail || err.message || 'Upload failed');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleProcess = async (id) => {
        try {
            ...
            setError(err.response?.data?.error || err.response?.data?.detail || err.message || 'Processing failed');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleActivate = async (id) => {
        try {
            ...
            setError(err.response?.data?.error || err.response?.data?.detail || err.message || 'Activation failed');
        } finally {
            setLoading(false);
        }
    };
        ...
        <Box>
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
                    ...
                    <Button
                        variant="outlined"
                        disabled={datasets.filter(d => d.status === 'uploaded').length === 0}
                        onClick={() => setTab(0)}
                    >
                        2. Validate & Process
                    </Button>
                    ...
                    <Button
                        ...
                        disabled={processedDatasets.length === 0}
                        onClick={() => {
                            setTrainForm({ ...trainForm, dataset_id: processedDatasets[0]?.id || '' });
                            setTrainDialog(true);
                        }}
                    >
                        4. Train Model
                    ...
                    <Button
                        ...
                        onClick={() => pendingModels.length > 0 && handleActivate(pendingModels[0].id)}
                    >
                        6. Publish Results
                    </Button>
                </Box>
                                    ...
                                    {datasets.map(ds => (
                                        <TableRow key={ds.id}>
                                            ...
                                            <TableCell>
                                                <Chip
                                                    label={ds.status}
                                                    color={STATUS_COLORS[ds.status] || 'default'}
                                                    size="small"
                                                />
                                            </TableCell>
                ...
                <TabPanel value={tab} index={1}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                        ...
                        <Button
                            ...
                            disabled={loading || processedDatasets.length === 0}
                        >
                            Train New Model
                        </Button>
                    </Box>
                    ...
                    {models.length === 0 ? (
                        <Alert severity="info">
                            No models trained yet. Process a dataset first, then train a model.
                        </Alert>
                    ) : (
                        <TableContainer>
                            <Table size="small">
                                ...
                                <TableBody>
                                    {models.map(m => (
                                        <TableRow key={m.id} sx={{ bgcolor: m.is_active ? '#e8f5e9' : 'inherit' }}>
                                            ...
                                            <TableCell>
                                                <Chip
                                                    label={m.is_active ? 'ACTIVE' : m.status}
                                                    color={m.is_active ? 'success' : STATUS_COLORS[m.status] || 'default'}
                                                    size="small"
                                                />
                                            </TableCell>
                ...
                <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
                    ...
                    <FormControl>
                        ...
                        <Select
                            ...
                            label="Dataset Type"
                            onChange={e => setUploadForm({ ...uploadForm, dataset_type: e.target.value })}
                        >
                    ...
                    <Alert severity="info" sx={{ mt: 1 }}>
                        Required columns vary by type. Example: population needs district, year, total_population. Sector and province are optional and may be left blank.
                    </Alert>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => { setUploadDialog(false); setSelectedFile(null); }}>
                        Cancel
                    </Button>
                ...
                <DialogContent>
                    {selectedDataset && (
                        <Grid container spacing={2} sx={{ mt: 1 }}>
                            <Grid item xs={6}>
                                <Typography variant="body2"><strong>Type:</strong> {selectedDataset.dataset_type}</Typography>
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="body2"><strong>Year:</strong> {selectedDataset.year}</Typography>
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="body2"><strong>Version:</strong> {selectedDataset.version}</Typography>
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="body2"><strong>Source:</strong> {selectedDataset.source || '—'}</Typography>
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="body2"><strong>Rows Processed:</strong> {selectedDataset.row_count || '—'}</Typography>
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="body2">
                                    <strong>Status:</strong>{' '}
                                    <Chip label={selectedDataset.status} size="small" color={STATUS_COLORS[selectedDataset.status] || 'default'} />
                                </Typography>
                            </Grid>
                            ...
                            {!selectedDataset.validation_errors && (
                                <Grid item xs={12}>
                                    <Alert severity="success">No validation errors found. Data quality is acceptable.</Alert>
                                </Grid>
                            )}
                        </Grid>
                    )}
